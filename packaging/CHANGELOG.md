...
## 0.4.0 — unreleased (target; PNR-3 second step, after the rc's clean-venv acceptance)
...
- Converting-honest coverage: publication state follows rule R-a (`live` only with zero
  ...
  `converting`; the site claims only `live` versions as supported (D2-06/D5-03).
...
## 0.4.0rc1 — unreleased (PNR-3 first step; pre-release — invisible to plain `pip install`)

...
def gates(server):
    # (name, argv, needs_server, skip_exit_codes)
    return [
        ("register",    _py(SELF / "verify_register.py"),                       False, ()),
        ("verdict",     _py(SELF / "verdict_gate.py"),                          False, ()),
        ("schema",      _py(SELF / "schema_oracle.py"),                         False, (2,)),
        ("suite-04-08", _py(CHK / "run_04_08.py"),                              False, ()),
        ("merchant",    _py(SELF / "validate_merchant_checks.py", "--server", server), True, ()),
        ("suite-01-23", _py(CHK / "run_01_23.py", server),                      True,  ()),
        ("killrate",    _py(SELF / "mutation_killrate.py"),                     True,  (2,)),
    ]
...
def main():
    ...
    results = []
    for name, argv, needs_server, skip_codes in gates(args.server):
        if name in skip:
            results.append((name, "SKIP", "explicitly skipped")); continue
        if needs_server and not up:
            if args.require_server:
                results.append((name, "FAIL", "golden server required but DOWN"))
            else:
                results.append((name, "SKIP", "no golden server"))
            continue

...
# ---------------------------------------------------------------------------------------
# I9 in PURE form (D1-16a's contract, kept here so the invariant text lives once — RV1 C14):
# merchant_checks_08_25_envelope.py (CHK-048 `idempotency.conflict_409`, CHK-078
# `idempotency.replay_cached`) imports these three by NAME with these signatures; the step
# form above (`replay_semantics`, I9 in INVARIANTS) is what seqfuzz_gate.py drives. Both
# cite the same anchors: checkout/rest.md#L1309-L1312 (CHK-048), checkout/index.md
# #L1109-L1118 (CHK-078/079), #L1028-L1029 (CHK-017). Inputs are plain status ints and
# decoded JSON bodies; each returns (ok: bool, detail: str).
...
#   terminal_rejected  a genuinely new Complete after a terminal state -> a 4xx error
#                      envelope, never a second order.

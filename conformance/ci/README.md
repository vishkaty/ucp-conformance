...
| `dormancy` | every merchant check runs on some golden or is named in `dormancy_exemptions.json` (floor 13; partial union is red) | the five goldens (+ the `golden-0825-mcp` record when :8195 boots) |
...
| `merchant-0825` | every merchant check is clean-pass + kill-safe on golden-0825 (:8197) against its PINNED skip population (`checks/expected_skips_golden_0825.json` carries the reviewed population); the four older merchant gates carry their own pinned files — an unexplained skip, a pinned id that ran, a class change or an expired clock is red on every golden | own golden-0825 |
| `merchant-mcp-only-0825` | the Shopify-shaped MCP-only 08-25 profile fixture (`fixtures/profiles/mcp_only_0825.py`, ephemeral loopback) grades against its pinned near-empty population (`expected_skips_mcp_only_0825.json`: 2 run / 226 pinned, every in-scope REST check `transport-not-declared`) and the CLI prints `coverage: null` + `support: rest-not-declared` — never a fake fraction for the largest fleet shape | own fixture |
| `req-kills-0825` / `req-kills-selftest` | every (check, register id) attribution at 2026-08-25 has >= 1 dedicated kill: a multi-id check declares `kills={id: [mutation…]}` and the run records (`merchant-0825`, `golden-check-08-25`, `struct-check-08-25`) show each declared kill observed; `shared` / `unkilled` are red (report-only at older versions until D1-21); the selftest drives a planted two-id check through the real runner | run records |
| `wave-targets` / `wave-targets-selftest` | every committed wave target file (`coverage/wave*_targets_*.json`, the ids a wave committed to convert) reads all CHECK at its version; mid-wave an id is excused only by a `blocked` entry naming a task id (`--allow-blocked`; the wave-close checkpoint runs without it) | — |
...
| `known-issues-page` | `public/known-issues.json`, `public/known-issues.html` and the `KI-*` rows of `site_claims.json` are byte-identical to a regeneration from `conformance/ci/known_issues.json` (`gen_known_issues.py --check`; the validator runs first, so a refuted or stale row refuses to render); `site-checkdocs` repeats the compare and `site-freshness` reds a published row older than 30 days | — |
| `deploy-guards` | `packaging/deploy.sh --selftest`: the only deploy path refuses a dirty tree, HEAD≠origin/main, a SHA with no completed-success `selftest` check-run (a failed run, a failure-only history, a green run under another job name), a stale site export or a red gate (exit 3); a newer in-progress run never masks an older green one (`packaging/check_run_verdict.py`, shared with `release_guards.sh`); deploys `preview-<sha7>` before `main` — synthetic repo + stub gh/wrangler with planted check-run histories | — |
| `preview-parity` / `preview-parity-selftest` | the JS instant check (functions/api/conformance.js, the badge) is mapped 1:1 to engine checks by the add-only id map `conformance/web/preview_parity.json` (projection `functions/api/preview_ids.js` byte-fresh); on the frozen capture set `conformance/web/preview_parity/captures/` (golden-0825, three controlled fixtures, the Flower Shop golden, 8 synthetic mutants) the JS verdicts and the real CLI's verdicts (each capture served from an ephemeral loopback stub) both equal `expected.json`, no JS/engine disagreement is untolerated, no tolerance is expired; the selftest plants a forced `capsOk`, a deleted id-map row and an expired tolerance | captured responses of servers we did not author + our own goldens |
| `docs-steward` | the steward docs (ARCHITECTURE.md, PRINCIPLES.md, DECISIONS.md, WORKSTREAMS.md, docs/archive/ROADMAP.md) are present, each `last-reviewed:` stamp is ≤ 90 days old, and DECISIONS.md carries one row per PLAN-v3 decision 1–34 + 4b with its CLOSED answer, date and who (`validate_steward_docs.py`; `--selftest` plants a missing doc, a missing 4b row and a 100-day-old stamp) | — |
| `discovery-shapes` | `discovery_shapes.py --selftest`: the pure discovery-live aggregation (shape buckets rest-keys / mcp-only / embedded-only, `version_2026_08_25`, `discovery_reach {stores, as_of}` over `discovery-capture/1` files with a 30-day window) reproduces its fixtures and reds on a planted 31-day-old capture; the ops scanner and the D4-08 sampler import it | — |

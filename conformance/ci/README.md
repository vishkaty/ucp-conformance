...
| `merchant-0825` | every merchant check is clean-pass + kill-safe on golden-0825 (:8197) against its PINNED skip population (`checks/expected_skips_golden_0825.json`: 30 run / 198 pinned after D1-15); the four older merchant gates carry their own pinned files — an unexplained skip, a pinned id that ran, a class change or an expired clock is red on every golden | own golden-0825 |

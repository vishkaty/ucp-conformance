...
_Generated 2026-08-12T11:59:17-0400 · methodology + fairness rules: see README.md in this directory. All numbers reproducible via `reproduce.sh`._

## Pinned inputs

- **spck_repo**: `9722cd2d1225d5a26fc40b97bec76e04f234863a+dirty`
...
- spck: aggregate `incomplete`, 30 applicable MUSTs clean-pass, 0 deviations
...
|---|---|---|---|---|---|---|
| disc-version-nondate | Discovery profile advertises a non-date version string ('draft') | OVR-010 | Y | MISS | **CATCH** — test_discovery, test_version_negotiation | yes |
...
| chk-missing-status | Checkout responses omit the required top-level `status` field | CHK-034 | Y | **CATCH** — checkout.complete_order, checkout.create_valid, checkout.response_fields, payment.handler_id_validated | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
| chk-missing-totals | Checkout responses omit the required top-level `totals` field | CHK-034 | Y | **CATCH** — discount.single_applied | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
...
| chk-status-bad-enum | Checkout `status` carries a value outside the six lifecycle values ('shipped') | CHK-033 | Y | **CATCH** — checkout.complete_order, checkout.create_valid, payment.no_credential_echo | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
| tot-subtotal-removed | No totals entry of type `subtotal` in checkout responses | TOT-005 | Y | MISS | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
| tot-total-removed | No totals entry of type `total` in checkout responses | TOT-006 | Y | MISS | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
| tot-discount-positive | Discount totals entries carry POSITIVE amounts (sign flipped) | TOT-014, DSC-021 | Y | MISS | **CATCH** — test_discount_flow, test_fixed_amount_discount, test_multiple_discounts_accepted, test_multiple_discounts_one_rejected | yes |
| tot-subtotal-negative | Subtotal totals entries carry NEGATIVE amounts (sign flipped) | TOT-015 | Y | MISS | **CATCH** — test_discount_flow, test_fixed_amount_discount, test_multiple_discounts_accepted, test_multiple_discounts_one_rejected | yes |
| tot-entry-missing-amount | Subtotal totals entries omit the required `amount` field | TOT-020 | Y | MISS | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
...
| err-http-200-always | Server returns HTTP 200 for every request it actually rejected (errors only signaled in the body) | OVR-014, CHK-017 | Y | **CATCH** — checkout.create_requires_line_items, checkout.idempotency_conflict, checkout.protocol_error_shape | **CATCH** — test_cannot_cancel_completed_checkout, test_cannot_complete_canceled_checkout, test_cannot_update_canceled_checkout, test_cannot_update_completed_checkout | yes |
| complete-missing-order | Complete-checkout response omits the populated `order` field | CHK-025 | Y | **CATCH** — checkout.complete_order, order.entity_shape, order.line_item_shape, order.response_valid_json | **CATCH** — test_complete_checkout, test_invalid_adjustment_status, test_malformed_adjustment_payload, test_order_adjustments | yes |
| order-missing-permalink | Order confirmation omits `permalink_url` | ORD-020 | Y | MISS | **CATCH** — test_complete_checkout | yes |
...
| chk-body-not-json | Checkout response bodies are not valid JSON (truncated by one byte) | CHK-044 | Y | **CATCH** — checkout.complete_order, checkout.create_valid, checkout.deterministic_logic, checkout.response_body_valid_json | **CATCH** — test_ap2_mandate_completion, test_token_binding_completion, test_buyer_consent, test_buyer_info_persistence | yes |
...
- disc-version-nondate: Discovery profile advertises a non-date version string ('draft') — caught by the official suite; tracked as a spck-suite gap.
- tot-subtotal-removed: No totals entry of type `subtotal` in checkout responses — caught by the official suite; tracked as a spck-suite gap.
- tot-total-removed: No totals entry of type `total` in checkout responses — caught by the official suite; tracked as a spck-suite gap.
- tot-discount-positive: Discount totals entries carry POSITIVE amounts (sign flipped) — caught by the official suite; tracked as a spck-suite gap.
- tot-subtotal-negative: Subtotal totals entries carry NEGATIVE amounts (sign flipped) — caught by the official suite; tracked as a spck-suite gap.
- tot-entry-missing-amount: Subtotal totals entries omit the required `amount` field — caught by the official suite; tracked as a spck-suite gap.
- order-missing-permalink: Order confirmation omits `permalink_url` — caught by the official suite; tracked as a spck-suite gap.

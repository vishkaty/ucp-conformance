...
all fixture-schema, all judged by _oracle_against/_oracle_root:
  ...
  DSC-021     MUST have negative amounts (one register requirement, two ids —
              discount.md's own DSC-021 states checkout/index.md's TOT-014
              rule for the discount type specifically -- same rule, different
              words, not a verbatim restatement; same schema
...
def _create_checkout_with_discount(codes):
    ...
    type "discount". The totals-discount-amount-not-negative mutant replaces the WHOLE `totals` array rather than setting an index, so it does not depend on this ordering."""
    body = {
        "line_items": [{"item": {"id": "bouquet_roses"}, "quantity": 1}],
        "fulfillment": _fulfillment_block(),
...
CHECKS = [
    ...
    Row("TOT-014", ["TOT-014", "DSC-021"], "fixture-schema",
        ...
        "ids, one check, matching CHK-034's own multi-id precedent. The "
        "sibling sentence at discount.md L319-320, that discounts.applied "
        "amounts are POSITIVE, is a different requirement and is carried by "

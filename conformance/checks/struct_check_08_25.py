...
oracle). NOTE (corrected 2026-09-02): this file's checks ARE attributed now. The
"land executable but UNATTRIBUTED" posture described here was true only for the first
batch; the 2026-08-25 attribution wall was subsequently lifted for this file (see
common/spec_versions.py), so every check added here flows straight into matrix.py /
the coverage export and onto the public site. Adding a check to this file is
therefore a PUBLIC COVERAGE CLAIM at the moment it lands — it must be kill-safe
against its own requirement, not merely against a sibling that subsumes it.
...
CATALOG_LOOKUP_ALGORITHM_CHECKS = [
    # CAT-015 gets its OWN check against dedupe_lookup_ids directly. Bound only
    # to the combined resolver below, its requirement was UNOBSERVABLE: that
    # resolver returns product keys, and its own product-level `seen_products`
    # set already collapses duplicate ids, so deleting the id-dedup step passed
    # every fixture (a surviving mutant) while CAT-015 still flipped to CHECK.
    # "Duplicate identifiers in the request MUST be deduplicated" (lookup.md
    # L56) is a distinct requirement from CAT-016's product-level rule (L58-59)
    # and is now tested where it is actually observable.
    Check("catalog.lookup_request_ids_deduplicated", ["CAT-015"],
          lambda ids, want: dedupe_lookup_ids(ids) == want,
          [(["sku1", "sku1", "sku2"], ["sku1", "sku2"]),
           (["a", "b", "a", "b"], ["a", "b"]),
           (["only"], ["only"])],
          # duplicates retained, order lost, or a survivor dropped
          [(["sku1", "sku1", "sku2"], ["sku1", "sku1", "sku2"]),
           (["a", "b", "a"], ["b", "a"]),
           (["a", "b", "a", "b"], ["a"])]),
    Check("catalog.lookup_merge_by_product", ["CAT-016"],
...
def compute_unit_price_amount(price_amount, measure_value, measure_scale,
    ...
    default round-half-to-even, which the prose does not forbid). The
    round-once half is exercised by the fractional-intermediate fixture below;
    the MODE is deliberately NOT pinned, so a truncating implementation is not
    treated as a violation and a truncation mutant legitimately survives.
...
CATALOG_UNIT_PRICE_CHECKS = [
    Check("catalog.unit_price_measure_reference_values_positive", ["CAT-039"],
          ...
          [(5000, 1), (1, 1)],
          # The non-integer negatives are what make the isinstance() half
          # load-bearing: with only integer negatives, dropping the type test
          # entirely still passes every fixture (a surviving mutant), while the
          # requirement is "MUST use a positive INTEGER" (index.md L104-105).
          [(0, 1), (5000, 0), (-1, 1), (1, -5), (2.5, 1), (1, 1.5)]),
    ...
    Check("catalog.unit_price_comparator_formula", ["CAT-042", "CAT-043"],
          ...
          [(7000, 5000, 2, 1, 0, 140),
           # A FRACTIONAL intermediate (measure = 25 x 10^-1 = 2.5 in the common
           # unit) is what makes the round-ONCE discipline load-bearing: every
           # other fixture divides exactly, so a mutant that rounds the scaled
           # measure first and the result again still matched. This one does not

...
def covered_ids_by_version():
    """Return {version: set(ids)} referenced by shipped checks, attributed per the rules above."""
    cov = {v: set() for v in VERSIONS}
    # gather MUST-row ids per version so we only attribute real rows
    must_ids = {v: {r.get("id") for r in load_rows(v)} for v in VERSIONS}
